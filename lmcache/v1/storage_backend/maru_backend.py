...
class MaruBackend(AllocatorBackendInterface):
    ...
    def __init__(
        ...
        self._rpc_errors_lock = threading.Lock()
        self._stats_monitor = LMCStatsMonitor.GetOrCreate()

        # 6. Handler capability flags (checked once at init)
        self._handler_supports_pin = hasattr(self._handler, 'pin')
        self._handler_supports_unpin = hasattr(self._handler, 'unpin')
    ...
    def rpc_errors(self) -> int:
        ...
        with self._rpc_errors_lock:
            return self._rpc_errors

    def _record_rpc_error(self) -> None:
        """Increment the RPC error counter in a thread-safe manner."""
        with self._rpc_errors_lock:
            self._rpc_errors += 1
    ...
    def _ensure_connected(self) -> bool:
        ...
        Thread-safe: uses double-checked locking to avoid concurrent
        reconnection attempts and handler leaks.
    ...
    def _mark_disconnected(self) -> None:
        """Mark connection as failed (thread-safe)."""
        with self._connection_lock:
    ...
    async def _async_store(
        ...
        try:
            if not self._ensure_connected():
                logger.error("[Maru] store skipped key=%s: not connected", key)
                return
            ...
            logger.error("[Maru] store timed out key=%s", key)
        except Exception as e:
            self._record_rpc_error()
            self._mark_disconnected()
    ...
    def get_blocking(
        ...
        try:
            ...
            logger.error("[Maru] get_blocking pin failed for key=%s", key)
            return None
    ...
    async def batched_async_contains(
        ...
        try:
            ...
            self._mark_disconnected()
            logger.warning(
                "[Maru] batched_async_contains failed for lookup_id=%s: %s",
                lookup_id, e,
            )
            return 0
    ...
    async def batched_get_non_blocking(
        ...
        def _release_partial(objs: list[MemoryObj]) -> None:
            """Release ref counts for partially-acquired objects."""
            for obj in objs:
                try:
                    obj.unpin()
                    obj.ref_count_down()
        ...
        try:
            ...
            self._mark_disconnected()
            logger.warning(
                "[Maru] batched_get_non_blocking failed for lookup_id=%s: %s",
                lookup_id, e,
            )
            return []
    ...
    def contains(self, key: CacheEngineKey, pin: bool = False) -> bool:
        ...
        try:
            ...
            self._record_rpc_error()
            self._mark_disconnected()
            logger.error("[Maru] contains failed for key=%s: %s", key, e)
            return False
    ...
    def unpin(self, key: CacheEngineKey) -> bool:
        ...
        """
        if self._handler_supports_unpin:
            try:
    ...
    def remove(self, key: CacheEngineKey, force: bool = True) -> bool:
        ...
        try:
            ...
            self._record_rpc_error()
            self._mark_disconnected()
            logger.error("[Maru] remove failed for key=%s: %s", key, e)
            return False
    ...
    async def healthcheck(self) -> bool:
        ...
        try:
            ...
            if not healthy:
                self._stats_monitor.update_remote_ping_error_code(
                    PING_GENERIC_ERROR_CODE)
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.error("[Maru] error closing memory allocator: %s", e)
        try:
            self._handler.close()
        except Exception as e:
            logger.error("[Maru] error closing handler: %s", e)
